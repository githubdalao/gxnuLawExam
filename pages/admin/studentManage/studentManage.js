import { getStudentList, deleteStudent, unBind, resetPwd } from '../../../service/admin'
...
Page({
  ...
  getList: function () {
    if (this.data.scrollFlag) {
      ...
      getStudentList(this.data.getListParam).then(res => {
  ...
  delete: function (e) {
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          deleteStudent({ uid: e.currentTarget.dataset.uid }).then(res => {
            Toast.success("删除成功");
          }).catch(err => {
            Toast.fail(err.msg);
          })
